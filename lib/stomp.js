...
var ServerFrame = {
  ...
  ERROR: function (socket, message, description, receipt, extraHeaders) {
 ...
 *
 * @return {Promise|undefined} settles once done when the handler was
 *   asynchronous, undefined when everything completed synchronously
 */
function whenDone(handler, onResult, onError) {
  ...
  if (result && typeof result.then === 'function') {
    return result.then(onResult).catch(onError);
  }
  onResult(result);
}
...
function fail(socket, message, description, receipt, extraHeaders) {
  try {
    ServerFrame.ERROR(socket, message, description, receipt, extraHeaders);
  } finally {
    socket.close();
  }
}
...
function FrameHandler(stompServer) {
  ...
  /**
   * Command handlers are called with (socket, frame). They return a Promise
   * when the command completes asynchronously (async middle-ware), so that
   * the next frame of the session waits for it.
   */
  this.CONNECT = function (socket, frame) {
    var version = negotiateVersion(frame.headers['accept-version']);
    if (version === null) {
      // `version` header and body as in the example of the specification
      return fail(socket, 'Supported protocol versions are ' + SUPPORTED_VERSIONS.join(','),
        'Supported protocol versions are ' + SUPPORTED_VERSIONS.join(' '), undefined,
        {version: SUPPORTED_VERSIONS.join(',')});
    }
  ...
  this.DISCONNECT = function (socket, frame) {
    ...
    return reply(socket, frame, function () {
