...
/**
 * Unescape a STOMP 1.1 header name or value. `\r` is a fatal error, as the
 * specification requires. Other unknown escape sequences are kept verbatim:
 * STOMP 1.0-style clients (e.g. stompjs 2.x) send backslashes unescaped, and
 * rejecting them would break those clients.
 */
function unescapeHeader(text) {
  ...
  return text.replace(/\\(.?)/g, function (match, chr) {
    switch (chr) {
      case 'n': return '\n';
      // undefined in STOMP 1.1; becomes valid (CR) with STOMP 1.2
      case 'r': throw new ProtocolError('Undefined escape sequence \\r');
      case 'c': return ':';
      case '\\': return '\\';
...
function addHeader(headers, line, unescape) {
  ...
  // a raw CR (the escape \r is rejected above) would let a sender corrupt
  // the header lines of subscribers (NUL is checked per block)
 ...
 * @param {number} [options.maxHeaders] headers per frame
 * @param {number} [options.maxHeaderLength] characters per header line
 */
function FrameDecoder(options) {
  options = options || {};
  this.maxFrameSize = options.maxFrameSize || DEFAULT_MAX_FRAME_SIZE;
  this.maxHeaders = options.maxHeaders || Infinity;
  this.maxHeaderLength = options.maxHeaderLength || Infinity;
  this._chunks = [];
  this._length = 0;
 ...
 *   (ws 8 delivers both kinds as Buffers); defaults to Buffer.isBuffer(chunk)
 */
FrameDecoder.prototype.push = function (chunk, isBinary) {
  var buf = Buffer.isBuffer(chunk) ? chunk : Buffer.from(String(chunk), 'utf8');
  if (isBinary === undefined) {
    isBinary = Buffer.isBuffer(chunk);
  }
  if (this._length === 0) {
    this._binary = false;
  }
...
FrameDecoder.prototype._parseHead = function (buf, version) {
  ...
  if (lines.length - 1 > this.maxHeaders) {
    throw new ProtocolError('Too many headers');
  }
  var unescape = Frame.shouldEscape(command, version);
  var headers = {};
  for (var i = 1; i < lines.length; i++) {
    if (lines[i].length > this.maxHeaderLength) {
      throw new ProtocolError('Header too long');
    }
